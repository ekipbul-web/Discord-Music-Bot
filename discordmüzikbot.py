...
# Eski kararlı yt-dlp sürümü için ayarlar
ytdl_opts = {
...
async def play_next(ctx):
    ...
    if q.queue:
        ...
        try:
            source = discord.FFmpegPCMAudio(q.current['url'], **ffmpeg_opts)
            ...
            await ctx.send(f"❌ Oynatma hatası, atlanıyor...")
            await play_next(ctx)
    else:
        q.current = None
...
async def play(ctx, *, query):
    ...
    # Link temizleme
    query = query.split('?si=')[0].split('&si=')[0].strip()
    ...
    try:
        ...
        if data.get('_type') == 'playlist' or len(entries) > 1:
            ...
            if count > 1:
                ...
                await msg.edit(content=f"✅ **1** şarkı sıraya eklendi!")
        else:
            # Tek şarkı
        ...
        await msg.edit(content=f"❌ **Hata:** Lütfen tekrar deneyin!")
...
async def show_queue(ctx):
    ...
    for i, song in enumerate(q.queue[:10], 1):
        ...
        embed.add_field(
            name=f"#{i}",
            value=f"**[{song['title']}]({song['webpage_url']})**{duration}",
            inline=False
        )
...
async def loop(ctx, mode: str = None):
    ...
    if mode is None:
        ...
        q.loop = 'off'
    elif mode.lower() in ['one', 'tek', '1', 'sarki']:
        q.loop = 'one'
    elif mode.lower() in ['all', 'tum', 'hepsi', 'liste']:
        q.loop = 'all'
    else:
...
@bot.command(name='stop', aliases=['durdurve'])
async def stop(ctx):
    if ctx.voice_client:
        q = get_queue(ctx.guild.id)
        q.queue.clear()
...
async def help_cmd(ctx):
    ...
    embed.set_footer(text="Kross Müzik Bot © 2026")
    await ctx.send(embed=embed)
