...
YETKILI_DM_KISI = "skayblaze"  # Görev teslim alacak yetkili
SPAM_LIMIT = 5  # saniye
SPAM_MESAJ = 3  # mesaj sayısı
...
def veri_yukle():
    ...
    return {
        ...
        "spam_kayit": {}, "link_kayit": {}, "dogum_gunleri": {}, "hatirlatmalar": []
    }
...
async def spam_temizle():
    veri = veri_yukle()
    simdi = datetime.now()
    for uid, kayitlar in list(veri["spam_kayit"].items()):
        kayitlar = [datetime.fromisoformat(k) for k in kayitlar if (simdi - datetime.fromisoformat(k)).total_seconds() < SPAM_LIMIT]
        veri["spam_kayit"][uid] = [k.isoformat() for k in kayitlar]
    veri_kaydet(veri)

# -------------------- OTO DM SİSTEMİ --------------------
@tasks.loop(minutes=1)
async def oto_dm_gonder():
    simdi = datetime.now()
    
    # Sabah 09:00 - Günaydın + Görev
    if simdi.hour == 9 and simdi.minute == 0:
        for guild in bot.guilds:
            for member in guild.members:
                ...
                try:
                    embed = discord.Embed(
                        title="🌅 Günaydın Kross Ailesi!",
                        description="Bugün harika bir gün olacak! İşte günlük görevin:",
                        color=0xFFD700
                    )
                    embed.add_field(name="🎯 Günlük 3 Üye Çekme Görevi", value="Sunucuya 3 yeni üye davet et!", inline=False)
                    embed.add_field(name="🏆 Ödüller", value="⭐ Özel Görev Rolü\n🎁 Sürpriz Hediye\n📊 Liderlik Puanı", inline=False)
                    embed.add_field(name="✅ Nasıl Yapacaksın?", value="1️⃣ Davet linkini paylaş\n2️⃣ 3 arkadaşını getir\n3️⃣ SS al, @skayblaze DM gönder", inline=False)
                    embed.add_field(name="⏰ Süre", value="Bugün 23:59'a kadar!", inline=False)
                    embed.set_footer(text="Kross Görev Sistemi • Her gün yeni görev!")
    ...
    if simdi.hour == 13 and simdi.minute == 0:
        for guild in bot.guilds:
            for member in guild.members:
                ...
                try:
                    embed = discord.Embed(
                        title="☀️ İyi Öğlenler!",
                        description="Günün yarısı geçti! Görevini tamamladın mı?",
                        color=0xFFA500
                    )
                    embed.add_field(name="🎯 Görev Hatırlatma", value="3 üye çekme görevi devam ediyor!", inline=False)
                    embed.add_field(name="💡 İpucu", value="Arkadaşlarını davet ederek sunucumuzu büyütmeye yardım et!", inline=False)
                    embed.set_footer(text="Kross Ailesi • Aktif kal!")
                    await member.send(embed=embed)
                    await asyncio.sleep(0.3)
                except: pass
    
    # Akşam 19:00
    if simdi.hour == 19 and simdi.minute == 0:
        for guild in bot.guilds:
            for member in guild.members:
                if member.bot: continue
                try:
                    embed = discord.Embed(
                        title="🌆 İyi Akşamlar!",
                        description="Akşam etkinliklerimiz başladı! Ses kanallarına bekliyoruz!",
                        color=0x8B0000
                    )
                    embed.add_field(name="🎤 Ses Kanalları", value="Müzik, sohbet ve oyun için ses kanallarına katıl!", inline=False)
                    embed.add_field(name="🎯 Görev", value="3 üye çekme görevi devam ediyor!", inline=False)
                    embed.set_footer(text="Kross Ailesi • İyi eğlenceler!")
                    await member.send(embed=embed)
                    await asyncio.sleep(0.3)
                except: pass
    
    # Gece 23:00
    if simdi.hour == 23 and simdi.minute == 0:
        for guild in bot.guilds:
            for member in guild.members:
                if member.bot: continue
                try:
                    embed = discord.Embed(
                        title="🌙 İyi Geceler!",
                        description="Gün bitti. Yarın yeni görevler seni bekliyor!",
                        color=0x000080
                    )
                    embed.add_field(name="📊 Bugün", value="Görevini tamamladın mı? Tamamlamadıysan yarın tekrar dene!", inline=False)
                    embed.add_field(name="🌅 Yarın", value="Saat 09:00'da yeni görev bildirimi alacaksın!", inline=False)
                    embed.set_footer(text="Kross Ailesi • Tatlı rüyalar!")
                    await member.send(embed=embed)
                    await asyncio.sleep(0.3)
                except: pass
...
async def on_member_join(member):
    ...
    try:
        ...
        embed.add_field(name="🎤 Ses Kanalları", value="Ses kanallarına katılarak sohbete başlayabilirsin!", inline=False)
        embed.add_field(name="🎯 Günlük Görev", value="3 üye çekme görevini tamamla, ödül kazan!", inline=False)
...
async def ban(ctx, member: discord.Member, *, reason: str = "Sebep belirtilmedi"):
    ...
    try:
        try:
            await member.send(embed=discord.Embed(title=f"🔨 {ctx.guild.name} Banlandınız!", description=f"Sebep: {reason}", color=0xFF0000).add_field(name="Yetkili", value=ctx.author.name))
        except: pass
...
async def oylama(ctx, sure: int = 0, *, soru: str):
    ...
    if sure > 0:
        ...
        sonuc = discord.Embed(title="🗳️ SONUÇ", description=soru, color=0x00FF00 if evet > hayir else 0xFF0000, timestamp=datetime.now())
...
async def yardim(ctx):
    embed = discord.Embed(
        ...
        description="Gelişmiş Güvenlik • Oto DM • Moderasyon",
        color=0x5865F2,
        timestamp=datetime.now()
    )
    ...
    embed.add_field(name="🤖 **Oto Sistemler**", value="🌅 Sabah/Öğle/Akşam/Gece DM\n⚡ Anti-Spam\n🔗 Anti-Reklam\n🚫 Anti-Küfür\n👋 DM Karşılama", inline=False)
...
if __name__ == "__main__":
    ...
    print("📨 Oto DM: 09:00 | 13:00 | 19:00 | 23:00")
    print("⚡ Anti-Spam: Aktif")
    print("🔗 Anti-Reklam: Aktif")
    print("🚫 Anti-Küfür: Aktif")
